...
//            contentScrollView.scrollEnabled = !(urlImageArray.count == 1)
            self.pageIndicator.frame = CGRectMake(0, self.frame.size.height - 30, kScreenWidth, 20)
...
//        self.setScrollViewOfImage()
        ...
        //分页指示器
        self.pageIndicator = UIPageControl(frame: CGRectMake(0, self.frame.size.height - 30, kScreenWidth, 20))
        pageIndicator.hidesForSinglePage = true
        pageIndicator.backgroundColor = UIColor.clearColor()
        pageIndicator.setValue(UIImage(named:"详情-轮播标记2" ), forKey: "pageImage")
        pageIndicator.setValue(UIImage(named:"详情-轮播标记1" ), forKey: "currentPageImage")
    ...
    private func setScrollViewOfImage(){
        ...
        let str = urlImageArray![indexOfCurrentImage]
        
        let curntUrl = NSURL(string:urlImageArray![indexOfCurrentImage] )
        
        self.currentImageView.sd_setImageWithURL(curntUrl)
        self.nextImageView.sd_setImageWithURL(NSURL(string:self.urlImageArray![self.getNextImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)]))
        self.lastImageView.sd_setImageWithURL(NSURL(string:self.urlImageArray![self.getLastImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)]))
