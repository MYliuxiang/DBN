...
//@property (nonatomic, retain)  CAShapeLayer *indicator;
//data source
...
- (void)animateIdicator:(CAShapeLayer *)indicator background:(UIView *)background tableView:(UITableView *)tableView title:(CATextLayer *)title forward:(BOOL)forward complecte:(void(^)())complete{
    ...
    for (int i = 0; i < _titles.count; i++) {
        ...
        CAShapeLayer *ind = (CAShapeLayer *)_indicators[i];
        ind.frame = CGRectMake(title.frame.origin.x + title.frame.size.width + 10, (self.frame.size.height - 10)/ 2 + 2,10, 10);
